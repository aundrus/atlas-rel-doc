...
def write_line(fhandle):
    ...
    if iline in recordLineNumber:
        x = """<div id=\"pb%s\" class=\"prblm\">%s</div>\n""" % (iline, x)
    fhandle.write(x)
...
#Sections with problem triggers are always written (10 lines above, 10 lines below)
#Last 2000 lines are also written
maxLines=100
topmaxLines=maxLines-15

optionDict={'conf':'CONF','config':'CONF'}
optionDict.update({'ext':'EXT','externals':'EXT'})
optionDict.update({'co':'CO','checkout':'CO','clone':'CO'})
optionDict.update({'im':'IM','image':'IM'})
#
optionDesc={'CONF':'cmake configuration', 'EXT':'externals build'}
optionDesc.update({'CO':'code checkout', 'IM':'image build'})
optionV=""; optionD=""
if option.lower() in optionDict:
    optionV=optionDict[option.lower()].lower()
    optionD=optionDesc.get(option,'build step '+optionV)
else:
...
errorList.append({'string':'CMake Error','ignore':['None'],'correlator':['None'],'logtype':['CONF','EX'],'flags':0})
errorList.append({'string':'cmake: command not found','ignore':['None'],'correlator':['None'],'logtype':['CONF','EX'],'flags':re.IGNORECASE})
errorList.append({'string':': error:','ignore':['None'],'correlator':['None'],'logtype':['CONF','EX'],'flags':0})
errorList.append({'string':'No rule to make target','ignore':['None'],'correlator':['None'],'logtype':['EX'],'flags':0})
errorList.append({'string':'ardoc_copy: problem','ignore':['None'],'correlator':['None'],'logtype':['CONF','IM'],'flags':re.IGNORECASE})
#
errorList.append({'string':'pNg','ignore':['None'],'correlator':['None'],'logtype':['CONF','IM'],'flags':re.IGNORECASE})
errorList.append({'string':'Foundd','ignore':['tdaq','fast'],'correlator':['.*9.*'],'logtype':['CONF','IM'],'flags':re.IGNORECASE})
#
warningList=[]
warningList.append({'string':'package not found','ignore':['None'],'correlator':['None'],'logtype':['ALL'],'flags':re.IGNORECASE})
warningList.append({'string':'CMake Warning','ignore':['None'],'correlator':['None'],'logtype':['CONF','EX'],'flags':0})
warningList.append({'string':'> Warning:','ignore':['None'],'correlator':['None'],'logtype':['CONF','EX'],'flags':0})
#
warningList.append({'string':'identification','ignore':['.*X.*'],'correlator':['--'],'logtype':['CONF','EX'],'flags':0})
minorWarningList=[]
minorWarningList.append({'string':'Warning: the last line','ignore':['None'],'correlator':['None'],'logtype':['ALL'],'flags':0})
#
minorWarningList.append({'string':'not available','ignore':['None'],'correlator':['a'],'logtype':['ALL'],'flags':0})
...
filehtml=filedir+os.sep+filebase_root+'.html'
if filedir == '': filehtml=filebase_root+'.html'
