...
#if len(sys.argv) < 2 :
#    print "ardoc_oracle_update_jobs_table: Error: no options specified\n"
#    sys.exit(1)
#try:
...
#if optStepName == "":
#    print "ardoc_oracle_update_jobs_table: Error: Step is not provided"
#    sys.exit(1)

nname=os.environ.get('ARDOC_NIGHTLY_NAME','')
current_projname=os.environ.get('ARDOC_PROJECT_NAME','')
parray=os.environ.get('ARDOC_PROJECT_ARRAY','UNDEF').strip()
parray_a=re.split(r'\s+',parray)
lnumber=len(parray_a)
if lnumber == 0:
    print("ardoc_oracle_update_jobs_table: Error: project array is empty")
    sys.exit(1)
first_project_name=parray_a[0]
#if current_projname == '' or current_projname != first_project_name:
#    print "ardoc_oracle_update_jobs_table: Info: project ", current_projname, "is not the first ",first_project_name, " ===>SKIPPING"
#    sys.exit(2)
scrind=os.environ.get('ARDOC_SCRATCH_INDICATOR','')
l_fullbuild=os.environ.get('ARDOC_FULLBUILD_LABEL','')
l_fullunit=os.environ.get('ARDOC_FULLUNIT_LABEL','')
l_fullintegr=os.environ.get('ARDOC_FULLINTEGR_LABEL','')
l_longtests=os.environ.get('ARDOC_LONGTESTS_LABEL','')
nprojb=os.environ.get('LEN_ARDOC_PROJECT_ARRAY','')
nprojmax=os.environ.get('LEN_ARDOC_PROJECT_ARRAY_MAX','')
print("ardoc_oracle_update_jobs_table: Info: SCRATCH_INDICATOR ",scrind)
print("ardoc_oracle_update_jobs_table: Info: FULLBUILD FULLUNIT FULLINTEGR LONGTESTS _LABELS  ",l_fullbuild, l_fullunit, l_fullintegr, l_longtests)
...
print("ardoc_oracle_update_jobs_table: Info: FULLBUILD FULLUNIT FULLINTEGR LONGTESTS CODES  ",i_fullbuild, i_fullunit, i_fullintegr, i_longtests)
print("ardoc_oracle_update_jobs_table: Info: NPROJ NPROJMAX ",nprojb,nprojmax)
...
if ci_results_dict != "" and os.path.isfile(ci_results_dict) :
    ...
    print("ardoc_oracle_update_jobs_table: Info: base name of CI_RESULTS_DICT: ",base_crd_0)
    os.chdir(dir_crd)
    print("ardoc_oracle_update_jobs_table: Info: CWD: ",os.getcwd())
    pm=[]
    try:
        pm = __import__(base_crd_0)
    except ImportError:
        traceback.print_exc()
        print("ardoc_oracle_update_jobs_table: Error: in importing generated results dict", base_crd_0)
        key_results_dict=0
        pass
...
print("ardoc_oracle_update_jobs_table: Info: build time dict: ",btt_dict)
print("ardoc_oracle_update_jobs_table: Info: unit time dict: ",utt_dict)
print("ardoc_oracle_update_jobs_table: Info: int time dict: ",ttt_dict) 
print("ardoc_oracle_update_jobs_table: Info: job time dict: ",jt_dict)
...
print("ardoc_oracle_update_jobs_table: Info: build times: ",bbj," # ",ebj)
print("ardoc_oracle_update_jobs_table: Info: unit times: ",buj," # ",euj)
print("ardoc_oracle_update_jobs_table: Info: int times: ",bij," # ",eij)
print("ardoc_oracle_update_jobs_table: Info: job times: ",bjj," # ",ejj)

warea=os.environ.get('ARDOC_WORK_AREA','')
if warea == "":
    print("ardoc_oracle_update_jobs_table: Error: ARDOC_WORK_AREA is not defined")
    sys.exit(1)
if not os.path.exists(warea):
    print("ardoc_oracle_update_jobs_table: Error: ARDOC_WORK_AREA:",warea," is not directory")
    sys.exit(1)
warea_dir=os.path.dirname(warea)
fjid=warea+os.sep+'jid_identificator'
if ( not os.path.isfile(fjid) ):
    print("ardoc_oracle_update_jobs_table: Error: file with jid does not exist:",fjid)
    sys.exit(1)
...
if jid == '':
    print("ardoc_update_jobs_table: Error: empty jid from",fjid)
    sys.exit(1)
...
try:
    ...
    print(("ardoc_oracle_update_jobs_table: Database connection error: ", error.code, error.offset, error.message))
else:
    print("ardoc_oracle_update_jobs_table: Connection is alive!")
...
if len(result) == 0:
    print("ardoc_oracle_update_jobs_table: Error: absent jid in jobs table:",jid)
    sys.exit(1)
rowmax=result[-1]
relid_current=rowmax[0]
nid_current=rowmax[1]
print("ardoc_oracle_update_jobs_table.py: nid,relid ",nid_current,relid_current)
...
if lresult > 0:
    for row in result:
         ...
         ntests_er=row[3]
         ntests_to=row[4]
         print("ardoc_oracle_update_jobs_table.py: ntests, project name ",ntests_p,name_p)
         ntests += int(ntests_p)
         ntestspb += int(ntests_pb)
         ntestser += int(ntests_er)
         nteststo += int(ntests_to)
print("ardoc_oracle_update_jobs_table.py: total number of tests",ntests,ntestspb,ntestser,nteststo)
...
print("ardoc_oracle_update_jobs_table.py: Oracle command", cmnd," # ",dict_p)
cursor.prepare(cmnd)
