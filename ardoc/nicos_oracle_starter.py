...
import datetime
import smtplib
print(("nicos_oracle_starter.py: Python version: " + platform.python_version()))
print(("nicos_oracle_starter.py: cx_Oracle version: " + cx_Oracle.version))
print(("nicos_oracle_starter.py: Oracle client: " + str(cx_Oracle.clientversion()).replace(', ','.')))
home=""
nname=""
ngroup="N/A"
valmode="N/A"
...
if nname == "":
    print("nicos_oracle_starter.py: Error: NICOS_NIGHTLY_NAME is not defined")  
    sys.exit(1)
oracle_schema=os.environ.get('NICOS_ORACLE_SCHEMA','ATLAS_NICOS').strip()
if 'NICOS_NIGHTLY_GROUP' in os.environ : ngroup=os.environ['NICOS_NIGHTLY_GROUP']
if 'NICOS_VAL_MODE' in os.environ : valmode=os.environ['NICOS_VAL_MODE']
if 'NICOS_INC_BUILD' in os.environ : incrmode=os.environ['NICOS_INC_BUILD']
if 'NICOS_PROJECT_ARRAY' in os.environ : parray=os.environ['NICOS_PROJECT_ARRAY']
scratch_indicator=os.environ.get('NICOS_SCRATCH_INDICATOR','').strip()
target_branch=os.environ.get('NICOS_TARGET_BRANCH','UNKNOWN').strip()
print("nicos_oracle_starter.py: Info: TARGET BRANCH: ",target_branch)
...
print("nicos_oracle_starter.py: date: ",ts," release name stamp: ",relnstamp)
#
nicos_http=os.environ.get('NICOS_HTTP','')
coma='bash -c \'(unset NICOS_SUFFIX;NICOS_PROJECT_NAME=\'PRJ\'; $NICOS_HOME/nicos_project_suffix_creator.pl)\''
sff=os.popen(coma,'r')
nicos_suffix=sff.readline()
#    print "SFF",nicos_suffix
nicos_webpage=nicos_http+'/'+ngroup+'WebArea/nicos_web_area'+nicos_suffix
print("nicos_oracle_starter.py: nicos_webpage: ",nicos_webpage)
#
...
print(("nicos_oracle_starter.py: Oracle DB version: " + connection.version))
#print ("nicos_oracle_starter.py: Oracle client encoding: " + connection.encoding)
...
try:
    ...
    print(("nicos_oracle_starter.py: Database connection error: ", error.code, error.offset, error.message))
else:
    print("nicos_oracle_starter.py: Connection is alive!")
...
if len(result) < 1: print("nicos_oracle_starter.py: absent nightly name: ",nname); sys.exit(1)
for row in result:
    print("nicos_oracle_starter.py:",row)
row=result[-1]
nightly_id=row[0]
statn=row[6]
stusern=row[7]
...
if role == 'master':
    mail_body="===============================================================\n"
    mail_body=mail_body+"NICOS restarted jobs for the nightly branch "+nname+"\n"
    ...
    if statn == 1 or statn == 3:
        subj = "NICOS restarted "+nname+" nightly"
...
""" % (", ".join(receivers), subj,mail_body)
        try:
            ...
            print("nicos_oracle_starter.py: SENDING MESSAGE TO ",receivers)
            print("--->",message)
        except:
            print("nicos_oracle_starter.py: Warning: unable to send email")
...
, :gitmrlink
, :relnmaster )"""
    dict_p={'nightly_id' : nightly_id,'relname' : relname,'reltype' : reltype,'tcrel' : tcrel,'tcrelbase' : tcrelbase, 'nicos_webpage' : nicos_webpage, 't_val' : ts, 'validation' : validation, 'relnstamp' : relnstamp, 'gitmrlink' : gitmrlink, 'relnmaster' : relnmaster}
    print("CMND", cmnd, " dict ",dict_p)
    cursor.prepare(cmnd)
    cursor.setinputsizes(t_val=cx_Oracle.TIMESTAMP)
    cursor.execute(None, dict_p)
    print('nicos_oracle_starter.py: bindvars ', cursor.bindvars)
ts_1=ts
if role != 'master':
    ts_1=ts - datetime.timedelta(days=2)
print('nicos_oracle_starter.py: tstamps:',ts,ts_1,role)
cmnd="""
...
if role == 'master':
    for row in result:
        if row[1] == ts:
            print("nicos_oracle_starter.py: relid found:",row[0],' XX ',row[1], ts)
    ...
    if lresult > 0:
        ...
        if rowmax[1] > ts_2:
            print("nicos_oracle_starter.py: relid found:",rowmax[0],' XX ',rowmax[1],' > ', ts_2)
            relid_j=rowmax[0]; ts_j=rowmax[1]
if relid_j == "":            
    print("nicos_oracle_starter.py: Error: relid for jobs table not found")
    sys.exit(1)
#    
tdd_j=ts_j.date()
...
print("nicos_oracle_starter.py: MASTER jid candidate (base)", jid)
####
if role != 'master':
    ts_j=ts
#
...
if lresult > 0:
    for row in result:
        print('nicos_oracle_starter.py: JID,RELID', row, jid)
        if row[0] > rowmax_max:
          rowmax_max = row[0] 
#    rowmax=result[-1]
    jid = rowmax_max
    print('nicos_oracle_starter.py: MAX_JID',jid) 
#    relid_jj = rowmax[1]
jid += 1
relid_jj=relid_j
print("nicos_oracle_starter.py: max jid:",jid," for relid:",relid_j,relid_jj," current ts:",ts_j," ROLE ",role_up)
#
cmnd="""
...
for row in result:
    ...
    if lresult > 0:
        jid = rowmax[0]
        print("nicos_oracle_starter.py: job id for jobstat insertion identified:",jid)
    else:
        print("nicos_oracle_starter.py: Error: absent job for relid:",relid_j)
        sys.exit(1)

if nicos_gen_config_area == '':
    print("nicos_oracle_starter.py: Error: absent nicos_gen_config_area")
    sys.exit(1)
fjid=nicos_gen_config_area+os.sep+'jobid.txt'
f=open(fjid, 'w')
f.write(str(jid))
f.close()
fepoch=nicos_gen_config_area+os.sep+'jobepoch.txt'
f=open(fepoch, 'w')
f.write(str(t_epoch))
f.close()
