...
if ($#ARGV < 1) {
warn "nicos_cachescan.pl: error: at least two arguments required\n";  
exit 1;
}
...
$opt="";
$section="";
$output="${NICOS_WORK_AREA}/cache_scan_result";
@conf_array=();
...
if ( $opt eq "" || $section eq "" ){
    warn "nicos_cachescan.pl: error: options -c <section> or -d <section> required\n";
    exit 1;
}

if ($#conf_array < 0){
    warn "nicos_cachescan.pl: error: nicos_cache files are not provided\n";
    exit 1; 
}
...
#	    print "LLLL $line\n";
	    ...
	    print TF "export ${line}\n";
          ...
          $label_bypass=1;
          print TF "if [ \"\${NICOS_BYPASS}\" != \"yes\" ]; then :;\n"; 
        }
