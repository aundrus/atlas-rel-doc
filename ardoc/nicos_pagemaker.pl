...
$total_tests=0;
$filenmb="${NICOS_WORK_AREA}/nicos_testprepage_number";
if ( -f $filenmb ){
    open (FKM, "$filenmb");
...
#		     print "QQQQ $testname $testname_base $testname_base2 WWWW @xxxx\n";
    $prg = "${NICOS_HOME}/nicos_wwwgen.pl -a ${testdir} ${testname_base1} ${testname_base2} ${testsuite} ${WTLogdir} ${recent_logfiles} ${problems} ${tecode} @xxxx";
    $pid = open(RM, "${prg} |");
    while (<RM>){ print WP "$_";}
    close (RM);
}
...
for (@addresses) { s/'/\\'/g }
  ...
  $prg = "${NICOS_HOME}/nicos_wwwgen.pl -g ${package} ${container} ${WLogdir} ${recent_logfiles} ${problems}  ${WQLogdir} ${qrecent_logfiles} ${qproblems} ${qecode} ${WTLogdir} ${trecent_logfiles} ${tproblems} ${tecode} @addresses";
  $pid = open(RM, "${prg} |");
  while (<RM>){ print WP "$_"; }
  close (RM);
...
system("cp -p ${NICOS_HOME}/doc/images/post-worldletter.gif ${WWW_DIR}/.");
system("cp -p ${NICOS_HOME}/doc/images/pur_sq.gif ${WWW_DIR}/.");
system("cp -p ${NICOS_HOME}/doc/images/question2_s.gif ${WWW_DIR}/.");
system("cp -p ${NICOS_HOME}/doc/images/rad18x16.gif ${WWW_DIR}/.");
system("cp -p ${NICOS_HOME}/doc/images/black_tick.gif ${WWW_DIR}/.");
system("cp -p ${NICOS_HOME}/doc/images/Master.gif ${WWW_DIR}/.");
system("cp -p ${NICOS_HOME}/doc/images/inform.gif ${WWW_DIR}/.");
system("cp -p ${NICOS_HOME}/doc/images/one_half.50x50.jpg ${WWW_DIR}/.");
system("cp -p ${NICOS_HOME}/doc/images/one_quarter.50x50.jpg ${WWW_DIR}/.");
system("cp -p ${NICOS_HOME}/doc/images/three_quarters.50x50.jpg ${WWW_DIR}/.");
...
if ( $NICOS_INC_BUILD eq "yes" )
{
    ...
    $filefile="${NICOS_WEBDIR}/${file_to_copy}";
    if ( -f $filefile ){
...
system("ln -s nicos_buildsummary.html index.html");
