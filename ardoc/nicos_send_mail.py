...
nicos_nightly_name=os.environ.get('NICOS_NIGHTLY_NAME','UNKNOWN')
nicos_arch=os.environ.get('NICOS_ARCH','UNKNOWN')
UID=os.getuid()
...
if len(optionslist) == 0:
  print('''Error: nicos_send_mail.py requires coomand line options  
                 Try "nicos_send_mail.py -h" for more information.''')
  sys.exit(2) 
level='NONE'
message='NONE'
...
mail_body="NICOS reports "+level+" ORACLE problems\n" 
mail_body=mail_body+"message "+message+"\n"
mail_body=mail_body+"hostname "+hostname+"\n"
mail_body=mail_body+"nightly name "+nicos_nightly_name+"\n"
mail_body=mail_body+"platform "+nicos_arch+"\n"
mail_body=mail_body+"date "+ts_jf+"\n"
mailing=True
if mailing:
          subj = "NICOS: "+level+" ORACLE problems on "+hostname
...
""" % (", ".join(receivers), subj,mail_body)
          try:
            ...
            print("nicos_send_mail.py: SENDING MESSAGE TO ",receivers)
            print("--->",message)
          except:
            print("nicos_send_mail.py: Warning: unable to send email")
            sys.exit(1)
