...
(sleep 1; \
 ...
 spp_check=`ps -ww -u $UID -o pid,ppid,args | grep -e "$command" | grep -v grep | grep -v "wrapper" | head -1 | awk '{print $1}'` ;\
 if [ "$spp" = "$spp_check" ]; then echo "[ARDOC, Oracle wrapper] Oracleinterface process killed as time quota spent"; ${ARDOC_HOME}/ardoc_kill_fam.py $spp 2; sleep 5; ${ARDOC_HOME}/ardoc_kill_fam.py $spp 9 ; fi;\
