...
if ($#ARGV < 1) {
    print "nicos_kill_fam: arguments required: process number(s), signal \n";
    exit 1;}
...
if ( $signal < 0 || $signal > 15 ) { 
   print "nicos_kill_fam: signal (last argument) must be between 0 and 15 \n";
   exit 1;}

for ($mmm=0; $mmm <= $fields; $mmm++){

my $procnum = $ARGV[$mmm];
print "nicos_kill_fam: handling process $procnum ... \n";
my $UID = $>;
$group_id;
my @pids;
my @procs1=();
