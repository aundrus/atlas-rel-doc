...
#
# ----------------------------------------------------------
# ardoc_cmake_loghandler_2ndloop
# ----------------------------------------------------------
#
sub compar{
    my $strg=shift;
...
my $ARDOC_CREATE_LOGFILES = "$ARDOC_CREATE_LOGFILES";
...
my $LOGHANDLER_REPORT="$ARDOC_WORK_AREA/ardoc_loghandler_2ndloop_report";
...
print RMNT "===========================================================================================\n";
print RMNT "[ATLAS ARDOC]: cmake build output that can not be assigned to a specific package\n";
print RMNT "===========================================================================================\n";
#warn "FFFFF @dbc\n";
...
#       remove trailing leading stretches whitespaces 
        ...
        if ( $cntr eq 1 && $stat eq 0 && $label_p eq 1 )
          ...
          $pkg_prev=$pkg;
          print "ardoc_cmake_loghandler_2ndloop: found separator for package $pkg\n";
          $foc=1;
          foreach $fo (@file_occurence){
          if ($fo eq $pkgn) {$foc=0;}  
          }
#          if ($ARDOC_CREATE_LOGFILES ne "no"){
          if ($foc eq 0 ){
	      $do_nothing=0;
#              open(OUT, ">> $file");
          } 
...
#          open(OUT, "> $file"); 
	  open(OUT, ">> $file");
          ...
          print OUT "===========================================================================\n";
          print OUT
"[ATLAS ARDOC]: build output for package $pkg extracted from cmake logfile\n";
	  print OUT "===========================================================================\n";
          close(OUT);#
#          } # if ($ARDOC_CREATE_LOGFILES ne "no")
          print REP "$pkg \n";
        ...
        if ($cntr >= 1) # && $ARDOC_CREATE_LOGFILES ne "no") 
        { 
	$file = "$LOGDIR/${pkgn}.loglog${file_suffix}";
        open(OUT, ">> $file"); 
...
$file = "$LOGDIR/${ARDOC_PROJECT_NAME}Release.loglog${file_suffix}";
print "ardoc_cmake_loghandler_2ndloop: adding unassigned output lines to $file\n";
open(OUT, ">> $file");
open(RMNT,"<$file_unassigned_lines");
print OUT while <RMNT>; 
close OUT;
close RMNT;
print "ardoc_cmake_loghandler_2ndloop: completed addition of unassigned output lines to $file\n";
