...
sub ardoc_mtime {
    ...
    if ($#ARGV != 0) {
    print "ardoc_relnum_calcul: one arg req: lifetime parameter \n";
    exit 1;}

    my $ARDOC_HOUR = "$ARDOC_HOUR";
    my $ARDOC_DAY_OFFSET = "$ARDOC_DAY_OFFSET";
    my $ARDOC_PROJECT_HOME = "$ARDOC_PROJECT_HOME";
    my $ARDOC_PROJECT_RELNAME_GEN = "$ARDOC_PROJECT_RELNAME_GEN";
    my $lifetime = $ARGV[0];
    my $relnum=0;
    my $lastsym="";
    ...
    opendir(DD, ${ARDOC_PROJECT_HOME});
    @allfiles = readdir DD;
    closedir DD;
  
    @list = grep /^${ARDOC_PROJECT_RELNAME_GEN}.+$/, @allfiles;
    $relnumb_max=-1;
    @nmbrs = ();
    $relnumb_maxlife=-1; 
    ...
    foreach $listfile (@list){
        ...
        if ($relnumb =~ /\d+$/){
        push(@nmbrs,$relnumb);  
        $path_to_file="${ARDOC_PROJECT_HOME}/${listfile}";  
        $file_mtime=ardoc_mtime($path_to_file);
...
#  if ($lifetime ne 7 && $ARDOC_DAY_OFFSET !=""){
  if ($ARDOC_DAY_OFFSET !=""){
  $timenum=$timenum + $ARDOC_DAY_OFFSET;  
  }
    if ( $ARDOC_HOUR =~ /\d+$/ ) {
    $hr = $ARDOC_HOUR;
    $hour = $curtime[2];
