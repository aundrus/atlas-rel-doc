...
#    my $relbase = $arr[0];   
    ...
    @wwwli = sort
    grep { /^nicos_${cont_word}summary_\d?\d?\d\.html$/ }
    readdir (WWWS);
    closedir(WWWS);
...
#        my $relnum2=$arr2[0];
        my $nnnn="<a href=\"${NICOS_WEBPAGE}/${_}\">${relbase}_${relnum2}</a>";
...
if ( $NICOS_BUILD_FROM_SCRATCH eq "yes" ){
print "Release: $rel_name &nbsp; -- &nbsp; Built from scratch on: ${NICOS_HOSTNAME}\n";
}
else{
print "Release: $rel_name &nbsp; -- &nbsp; Built on: ${NICOS_HOSTNAME}\n";
}
...
$m_image="";
$tm_image="";
$filem="${NICOS_WEBDIR}/status_email${relnum_suffix}";
$filetm="${NICOS_WEBDIR}/status_test_email${relnum_suffix}";
if ( -f $filem ){
open (FK, "$filem");
...
<a href=\"${NICOS_WEBPAGE}/nicos_content${relnum_suffix}.html\">list of tags</a>
</td>
...
(click for <a href=\"nicos_testsummary${relnum_suffix}.html\">details</a> or <a href=\"${NICOS_COMMON_WEBPAGE}/ATNSummary_${NICOS_PROJECT_RELNUMB_COPY}.html\" target=\"cumulative test list\">cumulative results</a>)
</th>
</tr>";
##<form>
...
print"
<tr><align=center >
<th bgcolor=\"CCCCFF\"><A href=\"nicos_buildsummary${relnum_suffix}.html\">packages names, failures in b/order first</A></th>
...
my $NICOS_WEB_HOME="$NICOS_WEB_HOME";
...
if ( $NICOS_WEB_HOME ne "" ){
push(@output,"<td><a href=\"$NICOS_WEB_HOME/$NICOS_PROJECT_RELNAME_COPY/$NICOS_INTTESTS_DIR/$work_dir\">link</a></td>\n");
}
if ( $a_names eq "" )
