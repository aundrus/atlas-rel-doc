...
if name == "":
    print(("nicos_oracle_add_node.py: Error: node ",name," is not defined"))  
    sys.exit(1)
...
try:
    ...
    print(("nicos_oracle_add_node.py: Database connection error: ", error.code, error.offset, error.message))
...
print(("nicos_oracle_add_node.py: Info: adding node ",name," id ",hostnam_id, "to table nodes"))
...
if lresult > 0:
    for row in result:
        ...
        if hid == hostnam_id and hname != name: 
            sign_dupl=True
            print(("nicos_oracle_add_node.py: Error: id ",hostnam_id," is assigned to another node ",hname))
        if hid != hostnam_id and hname == name: 
            sign_dupl=True
            print(("nicos_oracle_add_node.py: Error: node ",hostnam_id," already exists in the table, with different id ",hid))
if sign_dupl: sys.exit(1)
if not sign_add:
    print(("nicos_oracle_add_node.py: Warning: node ",hostnam_id," already exists in the table,  id ",hid))
#
cmnd="""
...
print(("nicos_oracle_add_node.py: Info: ADDED node ",name," id ",hostnam_id, "to table nodes"))
