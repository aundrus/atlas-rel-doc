...
#    $dirlog="${ARDOC_RELHOME}/build.${ARDOC_ARCH}/${pkg}";
    $dirlog="${ARDOC_RELHOME}/build.${ARDOC_ARCH}/BuildLogs";
    if ( ! -d $dirlog && -d "${ARDOC_RELHOME}/BuildLogs" ) { $dirlog="${ARDOC_RELHOME}/BuildLogs";}
    ...
    if ($#listlog < 0){
        open(OUT, "> $file");
        print OUT "===========================================================================\n";
#        print OUT "[ATLAS Nightly System]: no build logfiles are found in build.${ARDOC_ARCH}/${pkg}\n";
        print OUT "[ATLAS Nightly System]: no build logfiles for ${pkg_base} are found in build.${ARDOC_ARCH}/BuildLogs\n";
	print OUT "===========================================================================\n";
...
#        print "LLLL2222222222: @listordered\n";
        open(OUT, "> $file");
        print OUT "===========================================================================\n"; 
#        print OUT "[ATLAS Nightly System]: logfile from the build.${ARDOC_ARCH}/${pkg}, added in the order of modification date\n";
        print OUT "[ATLAS Nightly System]: logfile build.${ARDOC_ARCH}/BuildLogs/${pkg_base}.log\n";
        print OUT "===========================================================================\n";
...
#                else {print OUT "$_\n";}
                print OUT "$_\n";
                if ( $line_cnt == $count && $_ !~ /^.*Package\s+build\s+succeeded.*$/ && "$ARDOC_PROJECT_NAME" ne "GAUDI" ) { print OUT "Warning: the last line does not indicate that the package build is successful\n";}
                if ( $line_cnt == $count && "$ARDOC_PROJECT_NAME" eq "GAUDI" ) { print OUT "Info: CMake does not print messages \'Package build succeeded\' for ${ARDOC_PROJECT_NAME} project \n";}
            } 
            close(IN);
        } 
        close(OUT);
