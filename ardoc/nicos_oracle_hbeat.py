...
try:
    ...
    print('''nicos_oracle_hbeat.py: Error: You tried to use an unknown option or the
...
if nname == "":
    print("nicos_oracle_hbeat.py: Error: NICOS_NIGHTLY_NAME is not defined")  
    sys.exit(1)
oracle_schema=os.environ.get('NICOS_ORACLE_SCHEMA','ATLAS_NICOS').strip()
paname=os.environ.get('NICOS_PROJECT_NAME','')
#
warea=os.environ.get('NICOS_GEN_CONFIG_AREA','')
if warea == "":
    print("nicos_oracle_hbeat.py: Error: NICOS_WORK_AREA is not defined")
    sys.exit(1)
if not os.path.exists(warea):
    print("nicos_oracle_hbeat.py: Error: NICOS_WORK_AREA:",warea," is not directory")
    sys.exit(1)
warea_dir=os.path.dirname(warea)
fjid=warea+os.sep+'jobid.txt'
if ( not os.path.isfile(fjid) ):
    print("nicos_oracle_hbeat.py: Error: file with jid does not exist:",fjid)
    sys.exit(1)
mf=open(fjid)
jid=mf.readline().strip()
mf.close()
if jid == '':
    print("nicos_oracle_hbeat.py: Error: empty jid from",fjid)
    sys.exit(1)
print("nicos_oracle_hbeat.py: JID read:",jid,"from:",fjid)
#
...
try:
    ...
    print(("nicos_oracle_hbeat.py: Database connection error: ", error.code, error.offset, error.message))
#else:
...
if len(result) == 0:
            print("nicos_oracle_hbeat.py: Error: absent project:",paname)
            sys.exit(1)
rowmax=result[-1]
projid_current=rowmax[0]
#
print("nicos_oracle_hbeat.py: updating for jid",jid,", proj",projid_current,", date",ts_j,", status:",opt_stat)
if opt_stat == 'NONE':
    cmnd="""
...
"""
    print("nicos_oracle_hbeat.py: HBEAT UPDATE: ALIVE")
    cursor.prepare(cmnd)
