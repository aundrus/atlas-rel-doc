...
if 'NICOS_TESTLOG' in os.environ :
...
if 'NICOS_TEST_DBFILE_GEN' in os.environ :
...
for x in lines_db:
    ...
    if os.path.isfile(testpath):
        ...
        print("nicos_testdb_generator: Info: copying: ",testpath, copypath)
        shutil.copy2(testpath, copypath)
        if re.search('unit-test',testname,re.IGNORECASE):
            print("nicos_testdb_generator: modifying file", copypath)
            os.system(nicos_home+os.sep+'unit_test_log_processor.pl '+copypath)
    else:
        print("nicos_testdb_generator: Warning: test log does not exist: ",testpath) 
    if os.path.isfile(timestamppath):
        copypath=testlogdir+os.sep+container+'___'+testname+'__'+testname+'__m.timestamp'
        print("nicos_testdb_generator: Info: copying timestamp: ",timestamppath, copypath)
        shutil.copy2(timestamppath, copypath)
    else:
        print("nicos_testdb_generator: Warning: timestamp does not exist: ",timestamppath)
    if os.path.isfile(exitcodepath):
        copypath=testlogdir+os.sep+container+'___'+testname+'__'+testname+'__m.exitcode'
        print("nicos_testdb_generator: Info: copying exitcode: ",exitcodepath, copypath)
        shutil.copy2(exitcodepath, copypath)
    else:
        print("nicos_testdb_generator: Warning: exitcode does not exist: ",exitcodepath)
dbfileres.close()
