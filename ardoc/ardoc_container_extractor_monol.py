...
if 'ARDOC_SOURCEHOME' in os.environ :
    ...
    print("ardoc_container_extractor: Error: ARDOC_SOURCEHOME is not defined")
    sys.exit(1)
if 'ARDOC_RELHOME' in os.environ :
    relhome=os.environ['ARDOC_RELHOME']
else :
    print("ardoc_container_extractor: Error: ARDOC_RELHOME is not defined")
    sys.exit(1)
if 'ARDOC_ARCH' in os.environ :
    arch=os.environ['ARDOC_ARCH']
else :
    print("ardoc_container_extractor: Error: ARDOC_ARCH is not defined")
    sys.exit(1)
if 'ARDOC_DBFILE' in os.environ :
    dbfile=os.environ['ARDOC_DBFILE']
else :
    print("ardoc_container_extractor: Error: ARDOC_DBFILE is not defined")
    sys.exit(1)

#searchdir=relhome+os.sep+'aogt8'
searchdir=sourcehome
if not os.path.exists(searchdir): 
    print(("ardoc_container_extractor: Error: search dir "+searchdir+" does not exist"))
    sys.exit(1)
...
if not os.path.exists(cmakelogdir):
    print(("ardoc_container_extractor: Error: cmake log dir "+cmakelogdir+" does not exist"))
    sys.exit(1)
...
for k,v in list(dict_pack.items()):
    ...
    if os.path.exists(blogfile):
       print(("ardoc_container_extractor: found package and container ",k,v," and log ",blogfile))
       dbfileres.write(v+os.sep+k+'  '+k+' nomail@cern.ch\n')
    else:
       print(("ardoc_container_extractor: found package and container ",k,v," but NO  LOGFILE"))
dbfileres.close()
