...
try:
    ...
    print('''nicos_oracle_results: Error: You tried to use an unknown option or the
...
print("nicos_oracle_results.py: START, stage: ",opt_complete)
...
if 'NICOS_NIGHTLY_NAME' in os.environ : nname=os.environ['NICOS_NIGHTLY_NAME']
if nname == "":
    print("nicos_oracle_results.py: Error: NICOS_NIGHTLY_NAME is not defined")  
    sys.exit(1)
oracle_schema=os.environ.get('NICOS_ORACLE_SCHEMA','ATLAS_NICOS').strip()
parray=os.environ.get('NICOS_PROJECT_ARRAY','UNDEF').strip()
parray_s=os.environ.get('NICOS_PROJECT_ARRAY_S','UNDEF').strip()
paname=os.environ.get('NICOS_PROJECT_NAME','UNDEF').strip()
...
role=os.environ.get('NICOS_NIGHTLY_ROLE','master')
if len(role) > 6: role=role[0:6]
relname=os.environ.get('NICOS_PROJECT_RELNAME_COPY','')
reltype="NIT"
relstbb=os.environ.get('NICOS_STABLE_RELEASE','')
if relstbb == 'yes': reltype="STB"
tcrel=os.environ.get('NICOS_ATLAS_RELEASE','')
tcrelbase=os.environ.get('NICOS_ATLAS_ALT_RELEASE','')
pfilegen=os.environ.get('NICOS_TEST_DBFILE_GEN','')
if pfilegen == "":
    print("nicos_oracle_results.py: Error: NICOS_TEST_DBFILE_GEN is not defined")
    sys.exit(1)
pfilegen_base=os.path.basename(pfilegen)
warea=os.environ.get('NICOS_WORK_AREA','')
if warea == "":
    print("nicos_oracle_results.py: Error: NICOS_WORK_AREA is not defined")
    sys.exit(1)
if not os.path.exists(warea):
    print("nicos_oracle_results.py: Error: NICOS_WORK_AREA:",warea," is not directory")
    sys.exit(1)
nicos_inttests_dir=os.environ.get('NICOS_INTTESTS_DIR','')
atn_workdir=os.environ.get('ATN_WORKDIR','old')
warea_dir=os.path.dirname(warea)
nicos_webpage=os.environ.get('NICOS_WEBPAGE','')
nicos_web_home=os.environ.get('NICOS_WEB_HOME','')
nicos_project_relname_copy=os.environ.get('NICOS_PROJECT_RELNAME_COPY','')
link_base=nicos_webpage+os.sep+'NICOS_TestLog_'+nicos_project_relname_copy
ctest_tailor_file=os.environ.get('NICOS_CTEST_TAILOR_LIST','')
gitlabTB=os.environ.get('gitlabTargetBranch','')
print("nicos_oracle_results.py: Info: gitlabTargetBranch:",gitlabTB)

fjid=warea+os.sep+'jid_identificator'
fjid_p=warea+os.sep+'jid_identificator_' + paname
if ( not os.path.isfile(fjid) ):
    print("nicos_oracle_results.py: Error: file with jid does not exist:",fjid) 
    sys.exit(1)
...
if jid == '':
    print("nicos_oracle_results.py: Error: empty jid from",fjid)
    sys.exit(1)

print("nicos_oracle_results.py: JID read:",jid,"from:",fjid)
...
for x in lines_db:
    ...
    if (item < 5 ):
       print('nicos_oracle_results.py (first 5): test #',item+1,'fname:',tname,'tname:',tname_final,'dir',tdir,'dir_new',tdir_new,'sui',tsui,'tlimit',tlimit,'cont',tcont,'aut',taut)
...
connection.module = 'cx_Oracle nicos_oracle_results.py'
...
print('nicos_oracle_results: session user,schema:',result[0])

cmnd="""select projid from projects where projname = :paname order by projid"""
cursor.execute(cmnd, {'paname' : paname })
result = cursor.fetchall()
if len(result) == 0:
    print("nicos_oracle_results.py: Error: absent project:",paname)
    sys.exit(1)
rowmax=result[-1]
...
result = cursor.fetchall()
if len(result) == 0:
    print("nicos_oracle_results.py: Error: absent jid in jobs table:",jid)
    sys.exit(1)
rowmax=result[-1]
relid_current=rowmax[0]
nid_current=rowmax[1]

print("nicos_oracle_results.py: projid,nid,relid",projid_current,nid_current,relid_current)
...
if len(result) > 0:
   print("nicos_oracle_results.py: Warning: TSTAT has already a record for jid,projid:",jid,projid_current)
#   print "nicos_oracle_results.py: EXIT"
#   sys.exit(0)
else:
    cmnd="""
...
( :projid_c
  ...
  , :ncompl, :pccompl, :npb, :ner, :pcpb, :pcer, :nto, :pcto
    ...
    print("nicos_oracle_results.py:", cmnd, "dict", dict_p)
    cursor.prepare(cmnd)
...
if ctest_tailor_file == "" :
    print("nicos_oracle_results.py: Warning: ctest_tailor_file is not defined")
else:
    if not os.path.isfile(ctest_tailor_file):
        print("nicos_oracle_results.py: Warning: ctest_tailor_file does not exist: ", ctest_tailor_file)
    else:
        print("nicos_oracle_results.py: Warning: ctest_tailor_file found: ", ctest_tailor_file)
        with open(ctest_tailor_file, 'rb') as fp:
            ...
            while True:
                ...
                objs.append(o)
            lenobjs=len(objs)
            print("nicos_oracle_results.py: Info: number of objects read from ctest_tailor_file: ", str(lenobjs))
            if lenobjs >= 2 :
...
for it9 in allowed_tests :
    i9+=1
    print("nicos_oracle_results.py: Info: from ctest_tailor_file: test allowed: ", str(i9), it9) 
i9=0
for it9 in excluded_tests :
    i9+=1
    print("nicos_oracle_results.py: Info: from ctest_tailor_file: test excluded: ", str(i9), it9)
i9=0
for it9 in actually_excluded_tests :
    i9+=1
    print("nicos_oracle_results.py: Info: from ctest_tailor_file: test actually excluded: ", str(i9), it9)
i9=0
for it9 in excluded_tests_mod :
    i9+=1
    print("nicos_oracle_results.py: Info: from ctest_tailor_file: test excluded (mod): ", str(i9), it9)
i9=0
for it9 in actually_excluded_tests_mod :
    i9+=1
    print("nicos_oracle_results.py: Info: from ctest_tailor_file: test actually excluded (mod): ", str(i9), it9)
         
if opt_complete == 0:
    cmnd="""
delete from testresults where jid = :jid and projid = :projid_c"""
    print("nicos_oracle_results.py: cleaning: ", cmnd) 
...
#    print 'CMND',cmnd 
      ...
      print("nicos_oracle_results.py: Warning: no insertion for pid=",pid_current)   
#
    if opt_complete != 1:
      cmnd="""
...
#      print cmnd
      ...
      if len(result) > 0:
        ...
        print("nicos_oracle_results.py: Error: absent test in tests table:",tcombin," pid=",pid_current)
        #sys.exit(1)
        continue
      excess_flag=0
      ...
      print("nicos_oracle_results.py: TID, tname, excess_flag:",tid_current,tname,excess_flag)  
#
      cmnd="""
      ...
      if ( item < 5 ):
        print("nicos_oracle_results.py:CMNDA(5 items)", cmnd, "tval", ts_j)
        print("nicos_oracle_results.py:CMNDA dict", dict_p)  
      cursor.prepare(cmnd)
      cursor.setinputsizes(t_val=cx_Oracle.TIMESTAMP)
      cursor.execute(None, dict_p)

if opt_complete == 1:
    print("nicos_oracle_results.py: processing all tests results")
    fprepage=warea+os.sep+'nicos_testprepage'
    lines_db=[]
    try:
        mf=open(fprepage)
        lines_db = mf.readlines()
...
#           nerr=nerr+1
       ...
       if ttime_beg != '' and ttime_beg != '0.0' and ttime_beg != 'N/A':
          ...
          if re.match("^\d+?\.\d+?$", ttime_beg):
              ...
              print("nicos_oracle_results: Warning: test begin epoch time is not float: ",ttime_beg,ttime_beg_f)
       tend92=''
       if ttime_end != '' and ttime_end != '0.0' and ttime_end != 'N/A': 
          ...
          if re.match("^\d+?\.\d+?$", ttime_end):
              ...
              print("nicos_oracle_results: Warning: test end epoch time is not float: ",ttime_end,ttime_end_f) 
       ...
       print("nicos_oracle_results: final test info",tname_rplc_1,'*',tname,"S",tsui,'H',file_html,'R',tresult,'E',tecode,'D',tdir,'B',tname_base,'T',ttime_beg,ttime_end,'A',addresses,'P',pttrns,'EXC',excess_flag) 
       print("nicos_oracle_results: test start and end stamps: ",tbeg92,' # ',tend92)
       ...
       test_warea_link=nicos_web_home+os.sep+nicos_project_relname_copy+os.sep+nicos_inttests_dir+os.sep+work_dir
...
######EXIT CODE HARDWIRED TO ZERO       
    ...
    if ntest > 0:
        ...
        pcto=100.*nto/ntest
    print('nicos_oracle_results.py: Info: N tests,prb,err % problems, % errors',ntest,nprblm,nerr,nto,pcpb,pcerr,pcto)
    cmnd="""
    ...
    print('nicos_oracle_results.py: Command:',cmnd,cmnd,"dict",dict_p)
    cursor.prepare(cmnd)
    cursor.setinputsizes(t_val=cx_Oracle.TIMESTAMP)
    cursor.execute(None,dict_p)
