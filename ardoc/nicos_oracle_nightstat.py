...
try:
    ...
    sys.stderr.write('''nicos_oracle_nightstat.py: Error: You tried to use an unknown option or the argument for an option that requires it was missing.\n''')
    sys.exit(0)
...
if nname == "NONE":
    sys.stderr.write("nicos_oracle_nightstat.py: Error: NICOS NIGHTLY NAME is not defined\n")  
    sys.exit(1)
...
connection.module = 'cx_Oracle nicos_oracle_nightstat.py'
...
try:
    ...
    sys.stderr.write("nicos_oracle_nightstat.py: Database connection error: " + str(error.code) + " " + str(error.offset) + " " + str(error.message) + "\n")
#else:
#    print "nicos_oracle_hbeat.py: Connection is alive!"
...
if len(result) == 0:
            sys.stderr.write("nicos_oracle_nightstat.py: Info: this nightly branch do not exist: " + str(nname) + "\n")
            sys.exit(2)
if len(result) > 1:
            sys.stderr.write("nicos_oracle_nightstat.py: Error: multiple rows for this nightly branch do exist:" + str(nname) + "\n")
            sys.exit(1)
...
if shortprint : 
  ...
  print("nicos_oracle_nightstat.py: nightly",nname," has type",btype,", status",stat)
if opt_stat == 'set' :
  if set_stat == '0' or set_stat == '1' or set_stat == '3': 
    ...
    if shortprint : 
      ...
      print("nicos_oracle_nightstat: ",cmnd,' nightly name: ',nname,' new stat: ',set_stat,'requestor (parameter stuser):',stat_user)  
    ...
    if shortprint : 
      ...
      print("nicos_oracle_nightstat.py: Warning: this stat value is not allowed and will not be set: ",set_stat) 
cursor.close()
