...
import datetime
print("nicos_oracle_tags.py: START")
...
if nname == "":
    print("nicos_oracle_tags.py: Error: NICOS_NIGHTLY_NAME is not defined")  
    sys.exit(1)
...
parray=os.environ.get('NICOS_PROJECT_ARRAY',paname).strip()
...
#        print "nicos_oracle_tags.py: Info: current project", paname, " is not the last (",parray_a[-1],")"
#        print "nicos_oracle_tags.py: Info: no db insertions"
#        sys.exit(0)
#if lnumber == 0:
...
if role != "master":
        print("nicos_oracle_tags.py: Info: job role is",role," : no db insertions")
        sys.exit(1)
...
pfilegen=os.environ.get('NICOS_DBFILE_GEN','')
if pfilegen == "":
    print("nicos_oracle_tags.py: Error: NICOS_DBFILE_GEN is not defined")
    sys.exit(1)
pfilegen_res=pfilegen+'_res'
if not os.path.isfile(pfilegen_res):
    print("nicos_oracle_tags.py: Error: tag file",pfilegen_res,"does not exist")
    sys.exit(1)
pfilegen_res_base=os.path.basename(pfilegen_res)
warea=os.environ.get('NICOS_WORK_AREA','')
if warea == "":
    print("nicos_oracle_tags.py: Error: NICOS_WORK_AREA is not defined")
    sys.exit(1)
if not os.path.exists(warea):
    print("nicos_oracle_tags.py: Error: NICOS_WORK_AREA:",warea," is not directory")
    sys.exit(1)
...
for p_item in parray_a:
    ...
    print("nicos_oracle_tags.py: Info: updating projects if current project is new: ", p_item_s)
    cmnd="""
...
result = cursor.fetchall()
if len(result) == 0:
        print("nicos_oracle_tags.py: Error: absent project:",paname)
        sys.exit(1)
rowmax=result[-1]
...
result = cursor.fetchall()
if len(result) == 0:
    print("nicos_oracle_tags.py: Error: absent combination: n_name,relname,reltype:",nname,relname,reltype)
    sys.exit(1)
rowmax=result[-1]
relid_current=rowmax[0]
nid_current=rowmax[1]
print("nicos_oracle_tags.py: RELID,NID,PROJID:",relid_current,nid_current,projid_current)
#
cursor.close()
connection.commit()
connection.close()
...
it1=0
for p_item in parray_a:
    print("nicos_oracle_tags.py: Info: updating packages and tags for project", p_item)
    connection = cx_Oracle.connect(accnt,pwf,clust)
    connection.clientinfo = 'python 2.6 @ home'
    connection.module = 'cx_Oracle test_NICOS.py'
    connection.action = 'TestNicosJob'
    ...
    try:
        ...
        error, = exception.args
        print(("nicos_oracle_tags.py: Next database connection error: ", error.code, error.offset, error.message))
    else:
        print("nicos_oracle_tags.py: Next connection is alive!")
    cursor.execute("ALTER SESSION SET current_schema = "+oracle_schema)    
    p_item_s=p_item.strip()
    cmnd="""
...
""" 
    ...
    result = cursor.fetchall()
    if len(result) == 0:
        print("nicos_oracle_tags.py: Error: absent project:",p_item_s)
        projid_current='N/A'
    else:    
        rowmax=result[-1]
        projid_current=rowmax[0]
    it1 += 1
    coma='bash -c \'(NICOS_PROJECT_NAME='+p_item_s+'; $NICOS_HOME/nicos_project_suffix_creator.pl)\''
    print("nicos_oracle_tags.py: COMA",coma)
    sff=os.popen(coma,'r')
    sff_res=sff.readline()
#    print "SFF",sff_res 
    warea_i=warea_dir+os.sep+'nicos_work_area'+sff_res
#    print "WA",warea_i
    file_i=warea_i+os.sep+pfilegen_res_base
    print("nicos_oracle_tags.py: FILE_I",file_i)
    lines_db=[] 
...
#        print "CMND", cmnd
        ...
        print("nicos_oracle_tags.py: RELID,NID,PID,PROJID,DOMAIN:",relid_current,nid_current,pid_current,projid_current,domain)
        cmnd="""
