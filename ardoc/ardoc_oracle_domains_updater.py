...
if projn == "":
    logging.warn("ardoc_oracle_domains_updater.py: Error: PROJECT NAME is not defined")
    sys.exit(1)
if ardoc_arch == "":
    logging.warn("ardoc_oracle_domains_updater.py: Error: ARCHITECTURE (\"ARDOC_ARCH\" var) is not defined")
    sys.exit(1)
if mr_id == "" :
    logging.warn("ardoc_oracle_domains_updater.py: Error: gitlabMergeRequestId is not defined")
    sys.exit(1)
if mr_iid == "" :
    logging.warn("ardoc_oracle_domains_updater.py: Error: gitlabMergeRequestIid is not defined")
    sys.exit(1)
if mr_tbr == "" :
    logging.warn("ardoc_oracle_domains_updater.py: Error: gitlabTargetBranch is not defined")
    sys.exit(1)
if ci_results_file == "" :
    logging.warn("ardoc_oracle_domains_updater.py: Error: CI RESULTS file (\"CI_RESULTS_DICT\" var) is not defined")
    sys.exit(1)
if projn != 'Athena' and projn != 'AthSimulation':
    logging.info("ardoc_oracle_domains_updater.py: RUNS FOR Athena and AthSimulation projects only")
    sys.exit(1)
...
if os.path.isfile(ctest_dict_file) :
   logging.info("ardoc_oracle_domains_updater.py: using ctest results pickle file '%s'" % (ctest_dict_file))
else:
   logging.warn("ardoc_oracle_domains_updater.py: Error: ctest results pickle file '%s' does not exist" % (ctest_dict_file))
   sys.exit(1)
if os.path.isfile(afdomainlist_file) :
   logging.info("ardoc_oracle_domains_updater.py: using affected domain list from '%s'" % (afdomainlist_file))
else:
   logging.warn("ardoc_oracle_domains_updater.py: Error: affected domain list file '%s' does not exist" % (afdomainlist_file))   
   sys.exit(1)
lines_afd=[]
try:
...
logging.info("ardoc_oracle_domains_updater.py: affected domains: '%s'" % (affected_domains))
ts=ts_now
...
if os.path.isfile(fjid) :
    ...
    f.close()
if jid_res == '':
    logging.warn("ardoc_oracle_domains_updater.py: Error: JOB ID could not be retrieved")
    sys.exit(1)
logging.info("ardoc_oracle_domains_updater.py: jid retrieved from file: '%s'" % (jid_res))
...
#        objs.append(o)
#    lenobjs=len(objs)
#    logging.info("ardoc_oracle_domains_updater.py: number of objects read from pickle file '%s'", str(lenobjs))
#    if lenobjs >= 1 :
...
#    i9+=1
#    logging.info("ardoc_oracle_domains_updater.py: test allowed: '%s' '%s'" % (str(i9),item))
#i9=0
#for item in excluded_tests :
#    i9+=1
#    logging.info("ardoc_oracle_domains_updater.py: test excluded: '%s' '%s'" % (str(i9),item))
#
#logging.info("ardoc_oracle_domains_updater.py: output_list: '%s'" % (output_list_read))
...
try:
    ...
    logging.warn("ardoc_oracle_domains_updater.py: Database connection error: %s %s %s" % ( error.code, error.offset, error.message))
else:
    logging.warn("ardoc_oracle_domains_updater.py: Connection is alive!")
cursor.execute("ALTER SESSION SET current_schema = "+oracle_schema)    

logging.info("ardoc_oracle_domains_updater.py: reading test results from DB")
cmnd="""
...
logging.info("ardoc_oracle_domains_updater.py: cmnd: '%s' '%s'" % (cmnd,{'jid_res':jid_res,'projn':projn}))
...
if lresult <= 1:
    logging.warn("ardoc_oracle_domains_updater.py: Warning: number of tests performed is less than 2, exiting")

dict_domain_results={}
for afd9 in affected_domains:
    logging.info("ardoc_oracle_domains_updater.py: checking DB info for domain '%s'" % (afd9)) 
    cmnd="""
...
#and gitbr = :gitbr and dcont = :dcont order by tdstamp asc"""
    logging.info("ardoc_oracle_domains_updater.py: cmnd: '%s' '%s'" % (cmnd,{'jid':jidminus30days,'gitbr':mr_tbr,'projn':projn,'dcont':afd9}))
...
#    logging.info("ardoc_oracle_domains_updater: total N tests %s" % (lresult))
    logging.info("ardoc_oracle_domains_updater: results stored for domain %s : %s" % (afd9,dict_per_domain_results))
    l_dict_per_domain_results=len(dict_per_domain_results)
    if l_dict_per_domain_results <= 1:
        logging.warn("ardoc_oracle_domains_updater.py: Warning: number results stored for domain %s is less than 2" % (afd9))
    for k,v in list(dict_per_domain_results.items()):
        if k in dict_domain_results:
            if dict_domain_results[k] < v: 
                logging.info("ardoc_oracle_domains_updater.py: updating combined domain results from %s to %s" % (dict_domain_results[k], v)) 
...
connection.commit()
connection.close()
logging.info("ardoc_oracle_domains_updater.py: combined domain results %s" % (dict_domain_results))
...
for k,v in list(dict_results.items()):
    if k in dict_domain_results:
        ...
        if v > domain_res:
            logging.warn("ardoc_oracle_domains_updater.py: Issue: test '%s' result '%s' domain result '%s'" % (k,v,domain_res))
            inconsistency_list.append('test:'+k+',result:'+str(v)+',dbdata:'+str(domain_res))
        else:
            logging.info("ardoc_oracle_domains_updater.py: test '%s' result '%s' domane result '%s'" % (k,v,domain_res)) 
    else:
        logging.warn("ardoc_oracle_domains_updater.py: Warning: no result stored for test '%s' ('%s')" % (k,v))
...
if npbs > 0:
    logging.info("ardoc_oracle_domains_updater.py: findings are forwarded to %s", cumulative_data_file)
    file_handle=open(cumulative_data_file, 'a')
    file_handle.write('#\n')
    data_line=ardoc_project_relname+"#project:"+projn+"#target:"+mr_tbr+"#issues:"+str(npbs)+"#inconsistencies:"+str(len_inconsistency_list)
    file_handle.write(data_line+'\n') 
    if len_inconsistency_list > 0:
        data_line=ardoc_project_relname+"#project:"+projn+"#target:"+mr_tbr+"#hostname:"+hostname+"#jid:"+jid_res  
        file_handle.write(data_line+'\n')
        data_line=ardoc_project_relname+"#project:"+projn+"#target:"+mr_tbr+"#n_domains:"+str(len_affected_domains)+"#domnames:"+' '.join(affected_domains)
        file_handle.write(data_line+'\n')
        for item76 in inconsistency_list:
            file_handle.write(ardoc_project_relname+"#project:"+projn+"#target:"+mr_tbr+"#inconsistency:"+item76+'\n')
    file_handle.close() 
if len_inconsistency_list > 0:
    ...
    mail_body=mail_body+"local work area, MR name "+local_work_area+' '+ardoc_project_relname+"\n"
    ...
    logging.info("ardoc_oracle_domains_updater.py: sending email notification--->")
...
""" % (", ".join(receivers),subj,mail_body)
    try:
        ...
        logging.info("ardoc_oracle_domains_updater.py: SENDING MESSAGE TO '%s'",receivers)
    except:
...
if len_inconsistency_list > 0 and len_affected_domains <= max_len_affected_domains:
  ...
  try:
      ...
      error, = exception.args
      logging.warn("ardoc_oracle_domains_updater.py: Database connection #2 error: %s %s %s" % ( error.code, error.offset, error.message))
  else:
      logging.warn("ardoc_oracle_domains_updater.py: Connection #2 is alive!")
  cursor.execute("ALTER SESSION SET current_schema = "+oracle_schema)
#
  iter=0
  for afd9 in affected_domains:
...
SELECT did,dname,dcont FROM DOMAINS where dcont = :dcont"""
    ...
    result=cursor.fetchall()
    if len(result) < 1: logging.error("ardoc_oracle_domains_updater.py: absent domain container name: ",afd9); sys.exit(1)
    row=result[-1]
    domid8=row[0]
    logging.info("--------") 
    logging.info("ardoc_oracle_domains_updater.py: updating DB for domain '%s' - '%s'" % (domid8,afd9))
    for x in inconsistency_list:
        ...
        if line_a >= 3:
            ...
            logging.info("ardoc_oracle_domains_updater.py: test: %s, result: %s, result for domain %s" % (testn5,rest5,domrest5))
            cmnd="""
...
jid = :jid_res and projname = :projn and fname = :testn5"""
            logging.info("ardoc_oracle_domains_updater.py: cmnd: '%s' '%s'" % (cmnd,{'jid_res':jid_res,'projn':projn,'testn5':testn5}))
            cursor.execute(cmnd,{'jid_res':jid_res,'projn':projn,'testn5':testn5})
            result=cursor.fetchall()
            if len(result) != 1: logging.error("ardoc_oracle_domains_updater.py: incorrect length of results: ",len(result)); sys.exit(1)
            row=result[-1]
            res8=row[0]
            name8=row[2]
            pname8=row[3]
...
#
            ...
            if iter == 1:
                ...
                logging.info("ardoc_oracle_domains_updater.py: cleaning: '%s' '%s'" % (cmnd, { 'projid' : projid8, 'jid' : jid_res }))
                cursor.execute(cmnd, { 'projid' : projid8, 'jid' : jid_res }) 
                logging.info("ardoc_oracle_domains_updater.py: tdomresults deleted rows count %s",cursor.rowcount)
#                                                                                                                            
...
:jid,:nid,:relid,:did,:tid,:pid,:projid,:dres,:tdstamp,:updflag
            ...
            logging.info("ardoc_oracle_domains_updater.py: cmnd: '%s' '%s'" % (cmnd,dict_p))
            cursor.prepare(cmnd)
