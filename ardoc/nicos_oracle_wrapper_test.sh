...
shift
command=$@
source $NICOS_HOME/NICOS_oracle_setup_test.sh
hostnm=`hostname`
...
echo "[NICOS, Oracle interface wrapper (test) start time] $start_test"
echo "[NICOS, Oracle interface wrapper (test) end time]" `date`
