...
sub container_extractor{
    ...
    for ($m=0; $m <= $#testd; $m++){
        $testj=join('/', @testd);
        $filfil="${NICOS_RELHOME}/${prefix_relhome}/${testj}/cmt/version.cmt";
        last if ( -f $filfil );
        pop @testd;
        }
...
my $NICOS_NIGHTLY_NAME = "$NICOS_NIGHTLY_NAME";
my $NICOS_ARCH="$NICOS_ARCH";
my $NICOS_FULL_ERROR_ANALYSIS="$NICOS_FULL_ERROR_ANALYSIS";

$NICOS_TESTLOGDIR=dirname(${NICOS_TESTLOG});
$NICOS_TESTLOGDIR_TEMP="$NICOS_TESTLOGDIR/temp";
$WTLogdir="NICOS_TestLog_${release}";
...
##         @listfiles = sort { -M $a <=> -M $b } @list if ($par == 1);
         ...
         @test_db_nn = ();
         print "nicos_errorhandler: number of tests in test_db $number_tests_db\n"; ## if ($par != 1);
         }
         }
...
#        print "MLF $mlf $#listfiles1 $listfile\n";
	...
	$testdir1="";
        ...
        $direc="${NICOS_RELHOME}/$prefix_relhome/$testdir1";
...
#} if par != 1
        $fecod="${NICOS_TESTLOGDIR}/${testdir2}_${testname2}.exitcode";
        $exitcode="N/A";
        if ( -f $fecod ){
            open (FK, "$fecod");
        ...
        my @strng1=();
	$lower_NFEA=lc("$NICOS_FULL_ERROR_ANALYSIS");
	...
	if ( $lower_NFEA ne "true" && $lower_NFEA ne "yes" && $testname !~ /unit-tests/ ){
	    ...
	    @strng1=`${NICOS_HOME}/nicos_errortester.pl -elst ${testname} ${release}`;
        ...
        $tproblems=0.5;
        print "nicos_testhandler.pl: test minor warnings in $testname !!!\n";
...
#	print "BBBBBB $testname1 $testname2 $testdir1 $testdir2\n";
   ...
   $var="${NICOS_TESTLOGDIR}/$listfile"; 
   $var_orig="${NICOS_TESTLOGDIR}/$listfile";
   $varvar="${NICOS_WEBPAGE}/NICOS_TestLog_${release}/${listfileh}";
...
push(@body," project $project , $nightly release $release ${NICOS_ARCH}. \n");
...
unshift (@body_prev," !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
unshift (@body_prev," NICOS combined error meassages from ${body_count_prev} tests of ${testname_subject_prev} \n");
unshift (@body_prev," !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
    }

print "nicos_errorhandler.pl: sending email about $body_count_prev pbs in $testname_subject_prev, size $#body_prev, file count $mlf\n";
...
#print "AAAAAA @addr_total_prev BBBBB @cont_addr\n";
    ...
    foreach $address (@addr_total_prev){
        if ( $address ne "" ){
           if ( $address =~ /@/  ){
if ( $address =~ somebody ){
print "nicos_errorhandler.pl: Problems with ${type} $testname_subject_prev !!! Mail is NOT sent to such $address code $nomail_t[0] \n";                            
}
else{
    @address_fin=();
...
#        print "ADDADD $add95,$add96,$add97\n";
	    ...
	    print "nicos_errorhandler.pl: address $add96 blacklisted for ${type} $testname_subject_prev\n";
        } else {
...
print "nicos_errorhandler.pl: Problems with ${type} $testname_subject_prev !!! Mail is sent to $address_after_blacklist code $nomail_t[0] \n";
    } else {
print "nicos_errorhandler.pl: Problems with ${type} $testname_subject_prev !!! Mail list is empty \n";
    }
if ( $nomail_t[0] eq 0 && $test_after_blacklist ne "" ){
    $from_address="atnight";
    $to_address="$address_after_blacklist";
    $subject="NICOS: ${project}: ${type} problems in ${nightly} ${release} ${NICOS_ARCH} with ${testname_subject_prev}";
...
#--------------------------------------
        $fec="${NICOS_TESTLOGDIR}/${testdir2}_${testname2}.exitcode";
        $exitcod="N/A";
        if ( -f $fec ){
	    open (FK, "$fec");
            $exitcod = readline(FK);
        ...
        $tstmp="${NICOS_TESTLOGDIR}/${testdir2}_${testname2}.timestamp";
        $time_beg="N/A";
        $time_end="N/A";
        if ( -f $tstmp ){
            open (FK, "$tstmp");
        ...
        print "===errorhandler: exit code,file,dir: ${exitcod} ${testdir2}_${testname2}.exitcode ${NICOS_TESTLOGDIR}\n";
...
foreach $lf (@listf){
        next if $lf =~ /^\.\.?$/;
        next if $lf =~ /~$/;
        $lf_full="${NICOS_TESTLOGDIR}/${lf}";
        $lf_size=stat($lf_full)->size;
        if ( $lf_size <= 4000000 ){
        system("echo 'NICOS NOTICE: THIS FILE IS ALSO AVAILABLE AT:' > ${WTLogdir_full}/${lf}");
        system("echo '${NICOS_TESTLOGDIR}/${lf}' >> ${WTLogdir_full}/${lf}");
##        system("cat ${lf_full} >> ${WTLogdir_full}/${lf}");
#        system("cp -pf ${lf_full} ${WTLogdir_full}/.");
        }else{
        system("echo 'NICOS NOTICE: THIS FILE IS TRUNCATED DUE TO LARGE SIZE' > ${WTLogdir_full}/${lf}");
        system("echo 'LARGER, POSSIBLY NOT TRUNCATED COPY IS ${NICOS_TESTLOGDIR}/${lf}' >> ${WTLogdir_full}/${lf}");
...
my $NICOS_MAIL_WARNINGS = "$NICOS_MAIL_WARNINGS";
my $NICOS_MAIL_MINOR_WARNINGS = "$NICOS_MAIL_MINOR_WARNINGS";
my $NICOS_MAIL_UNIT_TESTS = "$NICOS_MAIL_UNIT_TESTS";
my $NICOS_MAIL_INT_TESTS = "$NICOS_MAIL_INT_TESTS";
my $NICOS_MAIL_PROJECT_KEYS = "$NICOS_MAIL_PROJECT_KEYS";
...
$nomail=1;
##if ( $NICOS_MAIL eq "yes" && $part_t eq "no" ){ $nomail=0; }
@nomail_t=(1, 1, 1);

$fileorig="${NICOS_DBFILE}";
$base_file=basename($NICOS_DBFILE);
$filename_gen="${NICOS_WORK_AREA}/${base_file}_gen";
...
$prepage_problems="nicos_prepage_problems";
$testprepage="nicos_testprepage";
...
#       remove trailing leading stretches whitespaces
	 ...
	 if ( $#fields > 0 ){
         ...
         @strng1=`${NICOS_HOME}/nicos_errortester.pl -es ${pkgn} ${release} ${filename} ${package}`; 
#         print "nicos_errorhandler.pl buildtest: @strng1 *** ${pkgbs} *** ${pkgn} VVV ${release} VVV ${filename} AAA @addr\n";  
...
#         print "nicos_errorhandler.pl buildtest: patt: @patterns \n";  
         $problems=0;
         if ( $fieldstr[0] eq "G" && $strng ne "" ) { 
         $problems=2;   
         print "nicos_errorhandler.pl: error level build problems with $pkgn of $release !!!\n";
         print "nicos_errorhandler.pl: offending pattern: @patterns\n";
         }
	 if ( $fieldstr[0] eq "W" && $strng ne "" ) {
	 $problems=1;
	 print "nicos_errorhandler.pl: warning level build problems with $pkgn of $release !!!\n";
         print "nicos_errorhandler.pl: offending pattern: @patterns\n";
	 }
         if ( $fieldstr[0] eq "M" && $strng ne "" ) {
	 $problems=0.5;
	 print "nicos_errorhandler.pl: minor warning level build problems with $pkgn of $release !!!\n";
	 print "nicos_errorhandler.pl: offending pattern: @patterns\n";
         }
...
#       remove trailing leading stretches whitespaces
        ...
        print "nicos_errorhandler.pl: $type[$i] minor warnings with ${pkgn} !!!\n";}         
...
#        print " ======= $listfilesf  =======\n";
  ...
  $listfilesh="$NICOS_LOGDIR/${pkgn}.html";
  system("cp -pf ${NICOS_LOGDIR}/${pkgn}.loglog ${NICOS_WEBDIR}/${WLogdir}/${pkgn}.html");
  system("cp -pf ${NICOS_LOGDIR}/${pkgn}.loglog ${NICOS_LOGDIR}/${pkgn}.html");
  $problems=2;
  $fieldstr[0]="G";
  ...
  print "nicos_errorhandler: generating missing logfile @fieldstr\n";
...
#   print " UUUUUUE $package $pkgcontainer $recent_logfile @addr \"\"\" $fieldstr[2] \"\"\" @patterns\n";
  ...
  if ( $totcount != 0 ){ 
    ...
    $percen=int($percen);
    if ( $percen > 50 ){ 
      print "nicos_errorhandler.pl: high percentage of packages with compilation problems : $percen : emails disabled\n";
      $nomail=2;
    } else {
      print "nicos_errorhandler.pl: percentage of packages with compilation problems : $percen\n";
    }
  }  
...
#     $drctr_copy="${NICOS_COPY_HOME}/${NICOS_PROJECT_RELNAME_COPY}/NICOS_area/${drctr_base}";
#   }
#   $var="${drctr_copy}/${pkgn}.loglog";
   $var="${drctr}/${pkgn}.loglog"; 
   $varvar="${NICOS_WEBPAGE}/${WLogdir}/${pkgn}.html"; 
   @body=();
  push(@body," ===========================================================\n");
  push(@body," This message is generated by NICOS build for \n");
  push(@body," project $project , $nightly release $release ${NICOS_ARCH}. \n");
  push(@body," NICOS system found possible $alarm_level for package \n $package\n");
...
#  print "MMMMM @body \n";
  ...
  $nomail_1=$nomail;
#  $nomail_1=-1;
  if ( $NICOS_MAIL_WARNINGS eq "no" && $alarm_level eq "warning signs" ){ $nomail_1=1; }
  if ( $NICOS_MAIL_MINOR_WARNINGS ne "yes" && $alarm_level eq "minor warning signs" ){ $nomail_1=1; }

  print "nicos_errorhandler.pl: Problems with $package !!! Mail is sent to $address code $nomail_1 (orig. $nomail ) \n";  
  if ( $nomail_1 eq 0 ){  
    $from_address="atnight";
    $to_address="$address";
    $subject="NICOS: ${project}: build problems in ${nightly} ${release} ${NICOS_ARCH} with ${package}";
#    $mailer = Mail::Mailer->new("sendmail");
#    $mailer->open({ From => "${from_address}",
#                      To => "${to_address}",
#                 Subject => "${subject}",
...
#    $dirtest_copy="${NICOS_COPY_HOME}/${NICOS_PROJECT_RELNAME_COPY}/NICOS_area/${dirtest_base}";
#    }
#  $var="${dirtest_copy}/${pkgn}.loglog";
  $var="${dirtest}/${pkgn}.loglog"; 
  $varvar="${NICOS_WEBPAGE}/${WTLogdir}/${pkgn}.html";
  @body=();
  $typet=$type[$i];
  push(@body," ===========================================================\n");
  push(@body," This message is generated by NICOS build for \n");
...
#  print "MMMMMM @body \n";
  ...
  if ( $nomail_t[$i] eq 0 ){
    $from_address="atnight";
    $to_address="$address";
    $subject="NICOS: ${project}: ${typet} problems in ${nightly} ${release} ${NICOS_ARCH} with ${package}";
#    $mailer = Mail::Mailer->new("sendmail");
#    $mailer->open({ From => "${from_address}",
#                      To => "${to_address}",
#                 Subject => "${subject}",
