...
logging.info("ardoc_oracle_test_results.py: START")
home=os.environ.get('HOME','')
nname=os.environ.get('ARDOC_NIGHTLY_NAME','')
arjid=os.environ.get('ARDOC_JOBID','')
t_epoch=os.environ.get('ARDOC_EPOCH','')
if arjid == "":
    logging.error("ardoc_oracle_test_results.py: Error: ARDOC_JOBID is not defined")
    sys.exit(1)
if t_epoch == "":
    logging.error("ardoc_oracle_test_results.py: Error: ARDOC_EPOCH is not defined")
    sys.exit(1)
if nname == "":
    logging.error("ardoc_oracle_test_results: Error: ARDOC_NIGHTLY_NAME is not defined")
    sys.exit(1)
...
#    sys.exit(1)
    if (item < 5 ):
       logging.info("ardoc_oracle_test_results.py (first 5): test # '%s' fname: '%s' tname: '%s' dir '%s' dir_new '%s' sui '%s' tlimit '%s' cont '%s' aut '%s'",item+1,tname,tname_final,tdir,tdir_new,tsui,tlimit,tcont,taut)
...
try:
    ...
    logging.warning("Database connection error: '%s' '%s' '%s'", error.code, error.offset, error.message)
else:
    logging.info("Connection is alive!")

cursor.execute("ALTER SESSION SET current_schema = '%s'",oracle_schema)
cmnd="""
...
#    print 'CMND',cmnd 
      ...
      logging.warning("ardoc_oracle_test_results.py: WARNING: no insertion for pid='%s'",pid_current)   
#
    cmnd="""
...
#    print cmnd
    ...
    if len(result) > 0:
        ...
        logging.warning("ardoc_oracle_test_results.py: Error: absent test in tests table: '%s' pid= '%s'",tcombin,pid_current)
        #sys.exit(1)
        continue
...
#    print "LLLLLL ",cmnd,projid_current,jid,tname_final
    ...
    if len(result) > 0:
        ...
        logging.warning("ardoc_oracle_test_results.py: Error: absent test in testresults table: '%s'",tname_final)
#        sys.exit(1)                                                                                 
