...
if len(sys.argv) < 2 :
    ...
    sys.exit(1)
try:
    optionslist, args = getopt.gnu_getopt(sys.argv[1:],'o:s:p:',['nochange','err=','suc=','war='])
except getopt.error:
...
logging.info("ardoc_oracle_jobstat_new_withwarn.py: Info: N errors, successes, warnings: '%s' '%s' '%s'",nerr,nsuc,nwar)
...
errors_successes=""
if stepDBN == 'LA': errors_successes=" , ER"+stepDBN+" = :nerr , SU"+stepDBN+" = :nsuc, WA"+stepDBN+" = :nwar" 
logging.info("ardoc_oracle_jobstat.py: error and success data:  '%s' '%s' '%s'",nerr,nsuc,nwar)
...
#       status code AND end date updated
        ...
        cursor.setinputsizes(t_val=cx_Oracle.TIMESTAMP)
        if stepDBN == 'LA':
          cursor.execute(None, {'relid_j': relid_j, 'jid' : jid, 'nightly_id' : nightly_id, 'projid_c' : projid_current, 't_val':ts_inp, 'statusCode' : statusCode, 'nerr' : nerr, 'nsuc' : nsuc, 'nwar' : nwar})
        else:
