...
try:
    ...
    print('''nicos_oracle_build_results: Error: You tried to use an unknown option or the
...
print("nicos_oracle_build_results.py: START, stage: ",opt_complete)
...
if nname == "":
    print("nicos_oracle_build_results.py: Error: NICOS_NIGHTLY_NAME is not defined")  
    sys.exit(1)
...
if warea == "":
    print("nicos_oracle_build_results.py: Error: NICOS_WORK_AREA is not defined")
    sys.exit(1)
if not os.path.exists(warea):
    print("nicos_oracle_build_results.py: Error: NICOS_WORK_AREA:",warea," is not directory")
    sys.exit(1)
...
link_base=nicos_webpage+os.sep+'NICOS_Log_'+nicos_project_relname_copy

fjid=warea+os.sep+'jid_identificator'
fjid_p=warea+os.sep+'jid_identificator_' + paname
if ( not os.path.isfile(fjid) ):
    print("nicos_oracle_build_results.py: Error: file with jid does not exist:",fjid) 
    sys.exit(1)
...
if jid == '':
    print("nicos_oracle_build_results.py: Error: empty jid from",fjid)
    sys.exit(1)

print("nicos_oracle_build_results.py: JID read:",jid,"from:",fjid)
...
connection.module = 'cx_Oracle nicos_oracle_build_results.py'
...
print('nicos_oracle_build_results: session user,schema:',result[0])
...
result = cursor.fetchall()
if len(result) == 0:
    print("nicos_oracle_build_results.py: Error: absent jid in jobs table:",jid)
    sys.exit(1)
rowmax=result[-1]
relid_current=rowmax[0]
nid_current=rowmax[1]

print("nicos_oracle_build_results.py: projid,nid,relid",projid_current,nid_current,relid_current)
...
if opt_complete == 0:
  if len(result) > 0:
    print("nicos_oracle_build_results.py: Warning: CSTAT has already a record for jid,projid:",jid,projid_current)
#    print "nicos_oracle_build_results.py: EXIT"
#    sys.exit(0)
  else:
    cmnd="""
...
jid = :jid and projid = :projid_c"""
    print("nicos_oracle_reljobs.py: completion:", cmnd, "t_val", ts_j)
    cursor.prepare(cmnd)
    cursor.setinputsizes(t_val=cx_Oracle.TIMESTAMP)
    cursor.execute(None, {'projid_c' : projid_current, 'jid' : jid ,'cstat' : 2,'t_val':ts_j})
  else: # if len(result) > 0:
    print("nicos_oracle_build_results.py: Warning: CSTAT does not have a record for jid,projid:",jid,projid_current)
    cmnd="""
...
( :projid_c
  ...
  , :ncompl, :pccompl, :npb, :ner, :pcpb, :pcer
    ...
    print("nicos_oracle_reljobs.py: completion:", cmnd, dict_p)
    cursor.prepare(cmnd)
    cursor.setinputsizes(t_val=cx_Oracle.TIMESTAMP)
    cursor.execute(None, dict_p)
#    
...
#
  ...
  fprepage=warea+os.sep+'nicos_prepage'
  lines_db=[]
  try:
      mf=open(fprepage)
      ...
      lines_db=[]
  order_db=[]
  forder=warea+os.sep+'nicos_comporder'
  try:
      mf=open(forder)
...
#          print "OOOOOOOOO", arr15[0], "++", arr_con, "++", ior 
          if arr15[0] == package and arr_con == container:
               print("nicos_oracle_build_results.py: Build order", ior, "for", arr_con, "/",arr15[0])
               c_order=ior+1
               break
...
#      print cmnd
      ...
      print("nicos_oracle_build_results.py: ",item,'p,cont:',package,container,'log:',recent_logfiles,'pbs:',cresult,qaproblems,uproblems,'adds:',addresses,'pttrns:',pttrns,'pid:',pid_current)
...
#      print 'LINK:',recent_logfiles_link
      if pid_current != '0':
        ...
        print("nicos_oracle_build_results.py: WARNING: no insertion for pid=",pid_current,package,container)   
...
#      , '0'
#      , '%(pttrns)s')""" % locals()
#      if item < 4:   print 'nicos_oracle_build_results.py: command for compresults insert',cmnd
#      cursor.prepare(cmnd)
#      cursor.setinputsizes(t_val=cx_Oracle.TIMESTAMP)
#      cursor.execute(None, {'t_val':ts_j})
  print("nicos_oracle_build_results: INFO: first 4 members of dict for compresults insertion:") 
  ...
  (:jid
   ...
   , :pttrns
   , :c_order)"""
  print("nicos_oracle_build_results: INFO: end of dict for compresults insertion:")
#  print 'FFFF',cmnd
  cursor.prepare(cmnd)
  cursor.setinputsizes(t_val=cx_Oracle.TIMESTAMP)
...
#
  ...
  print('nicos_oracle_build_results.py: Info: N packages, % problems, % errors',npack,pcpb,pcerr)   
  if ciresults_dir != '':
    if os.path.isdir(ciresults_dir):
       ...
       print('nicos_oracle_build_results.py: wrting into '+filecproblems+' '+str(nprblm)+' '+str(nerr))
  ...
  print('nicos_oracle_build_results.py: Command:',cmnd,dict_p)
