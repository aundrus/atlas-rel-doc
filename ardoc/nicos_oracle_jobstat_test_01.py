...
if len(sys.argv) < 2 :
    print("nicos_oracle_jobstat: Error: no options specified\n")
    sys.exit(1)
try:
...
if optStepName == "":
    print("nicos_oracle_jobstat.py: Error: Step is not provided")
    sys.exit(1)
...
if optStepName.lower() in stepDict:
    ...
    print("nicos_oracle_jobstat.py: Error: Unknown step provided:",optStepName)
    sys.exit(1)
print("nicos_orackle_jobstat: option:",stepDBN," ,statusCode:",statusCode)
...
if nname == "":
    print("nicos_oracle_interface_script.py: Error: NICOS_NIGHTLY_NAME is not defined")  
    sys.exit(1)
...
if lnumber > 1 and paname != "" :
    if parray_a[0] != paname:
        print("nicos_oracle_jobstat.py: Info: current project", paname, " is not the first (",parray_a[0],"): limited db insertions")
        firstproj=0
...
try:
    ...
    print(("nicos_oracle_jobstat.py: Database connection error: ", error.code, error.offset, error.message))
else:
    print("nicos_oracle_jobstat.py: Connection is alive!")
...
#pprint(result)
if len(result) < 1: print("nicos_oracle_jobstat.py: absent nightly name: ",nname); sys.exit(1)   
row=result[-1]
nightly_id=row[0]
...
#print 'nicos_oracle_jobstat.py: timestamps',ts,ts_1    
cmnd="""
...
#        print row
#        if row[1] == ts:
#            print "nicos_oracle_jobstat.py: relid found:",row[0],' XX ',row[1], ts
#            relid_j=row[0]; ts_j=row[1]
#            break
#else:
if lresult > 0:
        for row in result:
            print("nicos_oracle_jobstat.py: RELEASES row:",row)
        rowmax=result[-1]
        ts_2=ts - datetime.timedelta(days=2)
        if rowmax[1] > ts_2:
            print("nicos_oracle_jobstat.py: relid found:",rowmax[0],' XX ',rowmax[1],' > ', ts_2)
            relid_j=rowmax[0]; ts_j=rowmax[1]
#
if relid_j == "":
      print("nicos_oracle_jobstat.py: Error: relid for jobs table not found")
      sys.exit(1)
#print "TSJ",ts_j
...
print("nicos_oracle_jobstat.py: MASTER jid candidate", jid)
####
...
result = cursor.fetchall()
print("nicos_oracle_jobstat.py: DB command", cmnd)
#cursor.execute(cmnd)
lresult=len(result)
if lresult > 0:
  for row in result:
    print('nicos_oracle_jobstat.py: result JID,RELID', row)
  rowmax=result[-1]
  jid = rowmax[0]
  relid_jj = rowmax[1] 
#  if firstproj == 1: jid += 1
  if firstproj == 1 and  role != 'master': ts_j=rowmax[2]
print("nicos_oracle_jobstat.py: jid relid ts: ",jid,relid_j,relid_jj,ts_j," ROLE ",role_up) 
if relid_jj == "" or relid_j != relid_jj:
    print("nicos_oracle_jobstat.py: Error: relid from jobs table differs from releases table",relid_jj," xx ",relid_j) 
#
...
if key_results_dict == 1:
     ...
     print("nicos_oracle_jobstat_test:BASE_CRD_0 ",base_crd_0)
     pm=[]
     try:
        pm = __import__(base_crd_0)
     except ImportError:
        print("nicos_oracle_jobstat_test: Error: in importing generated results dict", base_crd_0)
        key_results_dict=0
        pass

print("nicos_oracle_jobstat_test: Info: KEY_RESULTS_DICT ", key_results_dict)      
...
#    if Q431TestTimeE != '0.' : arrIE.append(float(Q431TestTimeE))
    ...
    for k92,v92 in list(dictS.items()):
        print('nicos_oracle_jobstat.py: Info: START: ', k92, v92) 
    for k92,v92 in list(dictE.items()):
        print('nicos_oracle_jobstat.py: Info:  END : ', k92, v92)
#    if BuildTime.has_key(keystart):
#         start92=BuildTime[keystart]
#         print "nicos_oracle_jobstat_test: Info: value keystart ", start92
#         fstart92=float(start92)
#         tstart92=datetime.datetime.fromtimestamp(fstart92)
#         print "nicos_oracle_jobstat_test: Info: value keystart ", tstart92
#    else:
#         print "nicos_oracle_jobstat_test: Info: no value keystart "
#         for k92,v92 in BuildTime.items():
#             print k92, v92 
#    if BuildTime.has_key(keyend):
#         end92=BuildTime[keyend]
#         print "nicos_oracle_jobstat_test: Info: value keyend ", end92
#         fend92=float(end92)
#         tend92=datetime.datetime.fromtimestamp(fend92)
#         print "nicos_oracle_jobstat_test: Info: value keyend ", tend92
...
result = cursor.fetchall()
if len(result) == 0:
            print("nicos_oracle_jobstat.py: Error: absent project:",paname)
            sys.exit(1)
rowmax=result[-1]
...
result = cursor.fetchall()
if len(result) == 0:
     print("nicos_oracle_jobstat.py: Error: attempt to update non-existing jobstat row: jid,nid,relid,projid",jid,nightly_id,relid_j,projid_current)
     sys.exit(1)
...
print("nicos_oracle_jobstat.py: updating for nightly",nname_sel,", release",relname_sel,", proj",projname_sel,", begdate",begdate_sel,"***(",lresult,")***")

arrStep=[ stepDBN ]
arrCode=[ statusCode ]
if stepDBN == 'B' : 
    print("nicos_oracle_jobstat.py: step B")
    arrStep=[ 'B', 'CO', 'EXT', 'CONF', 'Q', 'I', 'Q431', 'Q221' ]
    if statusCode == "" :
        arrCode=[ statusCode, '', '', '', '', '' , '', '' ]
    else:
        arrCode=[ statusCode, '0', '0', '0', '0', '0' , '0', '0' ]
print("nicos_oracle_jobstat.py: ", arrStep)
i=0
for step in arrStep:
    ...
    code=arrCode[i]
    print("nicos_oracle_jobstat.py: starting update step: ", step, fstart92, tstart92, " ** ", fend92, tend92, " *CODE*",code,"*")
    i=i+1
    procid='100000'
...
#        ts_inp92=ts_inp
        ts_inp92="NULL"
        if tstart92 != "" :
          print("nicos_oracle_jobstat_test.py: Info: taking start time from results file:",tstart92, "--- was ",ts_inp)
          ts_inp92=tstart92
        else:
          print("nicos_oracle_jobstat_test.py: Warning: no start time retrieved from results file, using ",ts_inp92)
        print("nicos_oracle_jobstat.py: command ",cmnd00)
        print("nicos_oracle_jobstat.py: parameters ",relid_j,jid,nightly_id,projid_current)
        ...
        if ts_inp92 != "NULL":
          ...
          print("nicos_oracle_jobstat.py: command ",cmnd100)
          cursor.execute(cmnd100, {'relid_j': relid_j, 'jid' : jid, 'nightly_id' : nightly_id, 'projid_c' : projid_current })          
    else:    
#        ts_inp92=ts_inp
        ts_inp92="NULL"
        if tend92 != "" :
          print("nicos_oracle_jobstat_test.py: Info: taking end time from results file:",tend92, "--- was ",ts_inp)
          ts_inp92=tend92
        else:
          print("nicos_oracle_jobstat_test.py: Warning: no end time retrieved from results file, using ",ts_inp92)
        if code == "nochange":
            cmnd=cmnd3+cmnd9
            if ts_inp92 != "NULL":
              cursor.prepare(cmnd)
            ...
            cmnd=cmnd2+cmnd9
            print("nicos_oracle_jobstat.py: command ",cmnd)
            print("nicos_oracle_jobstat.py: parameters ",relid_j,jid,nightly_id,projid_current,ts_inp92,code) 
            if ts_inp92 != "NULL": 
              cursor.prepare(cmnd)
              cursor.setinputsizes(t_val=cx_Oracle.TIMESTAMP)
              cursor.execute(None, {'relid_j': relid_j, 'jid' : jid, 'nightly_id' : nightly_id, 'projid_c' : projid_current, 't_val':ts_inp92, 'code' : code})

    print("nicos_oracle_jobstat.py: finishing update step: ", step, "code:",code,"tval:", ts,"jid,projid:",jid,projid_current)
