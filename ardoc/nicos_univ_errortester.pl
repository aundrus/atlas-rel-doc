...
$NICOS_LOGDIRBASE = basename(${NICOS_LOGDIR});
#
...
if ( $filehtml ne "" ){
  warn "nicos_copy_errortester.pl: HTML: $filehtml\n";
  open(FG,">$filehtml");
  ...
  if ( $NICOS_WEBPAGE ne "" ){
      $webloc="${NICOS_WEBPAGE}/NICOS_Log_${NICOS_PROJECT_RELNAME_COPY}/$filebase1";
#      print FG "<BR>\n";
  ...
  warn "nicos_copy_errortester.pl: conversion info: @{$allowed_1[0]} -- @{$allowed_1[1]} \n";
  warn "nicos_copy_errortester.pl: conversion info: @{$allowed_2[0]} -- @{$allowed_2[1]} \n";
  $ncount_line=0;
  $ncount_s[0]=0;
  open(FB,"<$filename");
  while (<FB>){
...
#warn "nicos_copy_errortester: cp -Rp $filehtml $copy_html\n";
