...
if ( $testtesting == 0 && $qatesting == 0 ){
    ...
    $type_in_url="c";
    $type_in_html="build";
    $ARDOC_TESTLOGDIR=dirname($ARDOC_LOG);
    if ( "$#ARGV" ne 3 ){
    ...
    $ARDOC_TESTLOGDIR=dirname(${ARDOC_QALOG}) if ($qatesting == 1);
...
$ARDOC_TESTLOGDIRBASE = basename(${ARDOC_TESTLOGDIR});
...
my $BUILD_FAILURE_PATTERN="$ARDOC_BUILD_FAILURE_PATTERN";
...
if ($qatesting == 1) {
    $TEST_SUCCESS_PATTERN="$ARDOC_QA_SUCCESS_PATTERN";
...
$ARDOC_TESTLOGDIR_TEMP="$ARDOC_TESTLOGDIR/temp";
$file_er="${ARDOC_TESTLOGDIR_TEMP}/${compname}_ERROR_MESSAGE";
$file_wa="${ARDOC_TESTLOGDIR_TEMP}/${compname}_WARNING_MESSAGE";
$file_su="${ARDOC_TESTLOGDIR_TEMP}/${compname}_SUCCESS_MESSAGE";
$dfgert=1;
...
#                    $varbase = basename($var);
                    print "G $compname $ARDOC_TESTLOGDIR $eeee";
                }
                if ( $short eq 0 ){
                    exit 2;
                }
...
#			$varbase = basename($var);
			print "G $compname $ARDOC_TESTLOGDIR $ssss";
		    }
		    if ( $short eq 0 ){
			exit 2;
		    }
...
#			$varbase = basename($var);
			print "W $compname $ARDOC_TESTLOGDIR $wwww";
		    }
		    if ( $short eq 0 ){
			exit 2;
		    }
...
================\n";
                    ...
                    print "====================================================================\n";
                        ...
                        print "W $compname $ARDOC_TESTLOGDIR $ssww";
                    }
                    if ( $short eq 0 ){
                        exit 2;
                    }
...
#                       $varbase = basename($var);
                        print "M $compname $ARDOC_TESTLOGDIR $wwww_minor";
                    }
                    if ( $short eq 0 ){
                        exit 2;
                    }
...
if ( $filehtml ne "" ){
#    warn "ardoc_errortester.pl: HTML: $filehtml\n";
    open(FG,">$filehtml");
...
#    warn "ardoc_errortester.pl: conversion info: @{$allowed_1[0]} -- @{$allowed_1[1]} \n";
#    warn "ardoc_errortester.pl: conversion info: @{$allowed_2[0]} -- @{$allowed_2[1]} \n";
    $ncount_line=0;
...
{
    if ( $short eq 0 ){
    print "ardoc_errortester.pl found problem/warning, logfile(s) : $file\n"
    }
}
