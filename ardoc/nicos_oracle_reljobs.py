...
if 'NICOS_WORK_AREA' in os.environ : warea=os.environ['NICOS_WORK_AREA']
if 'NICOS_NIGHTLY_NAME' in os.environ : nname=os.environ['NICOS_NIGHTLY_NAME']
if nname == "":
    print("nicos_oracle_reljobs.py: Error: NICOS_NIGHTLY_NAME is not defined")  
    sys.exit(1)
...
if lnumber > 1 and paname != "" :
    ...
    if parray_a[0] != paname:
        print("nicos_oracle_reljobs.py: Info: current project", paname, " is not the first (",parray_a[0],")")
        print("nicos_oracle_reljobs.py: Info: limited db insertions")
        firstproj=0
    if parray_a[-1] == paname:
        lastproj=1
target_branch=os.environ.get('NICOS_TARGET_BRANCH','UNKNOWN').strip()
print("nicos_oracle_reljobs.py: Info: TARGET BRANCH: ",target_branch)
...
validation=os.environ.get('NICOS_VAL_MODE','')
nightly_group=os.environ.get('NICOS_NIGHTLY_GROUP','N/A')
nightly_name=os.environ.get('NICOS_NIGHTLY_NAME','N/A')
buildarea1=os.environ.get('NICOS_PROJECT_HOME','')
buildarea=os.path.dirname(buildarea1)
copyarea1=os.environ.get('NICOS_COPY_HOME','')
...
btool='CMAKE'
bvers=cmakevers
url2log=os.environ.get('NICOS_WEBPAGE','')
...
nicos_kitb=os.environ.get('NICOS_KIT_TOOL','void')
nicos_cvmfs=os.environ.get('NICOS_CVMFS_TOOL','void')
nicos_webpage=os.environ.get('NICOS_WEBPAGE','')
nicos_webarea=os.path.dirname(nicos_webpage)
nicos_http_build=os.environ.get('NICOS_HTTP_BUILD','')
nicos_suffix=os.environ.get('NICOS_SUFFIX','')
gitmrlink=os.environ.get('MR_GITLAB_LINK','')
gitmrhash=os.environ.get('MR_GITHASH','')
kitb = (0,1)[nicos_kitb.lower().strip() != 'no' and nicos_kitb.lower().strip() != 'void' ]
#print "KBBBB",nicos_kitb.lower().strip(),kitb
cvmfs = (0,1)[nicos_cvmfs.lower().strip() == 'yes']
#coma='bash -c \'if [ -f $NICOS_WORK_AREA/nicos_errorhandler_gen ]; then source $NICOS_WORK_AREA/nicos_errorhandler_gen >/dev/null 2>&1; echo \"${NICOS_MAIL};${NICOS_MAIL_INT_TESTS};${NICOS_MAIL_QA_TESTS}\"; fi\''
#sff=os.popen(coma,'r')
...
if os.path.isfile(fjid) :
    ...
    jid_res=f.readline()
    f.close()
print("nicos_oracle_reljobs.py: jid retrieved from file: ",jid_res)    
####if role == 'master'
...
try:
    ...
    print(("nicos_oracle_reljobs.py: Database connection error: ", error.code, error.offset, error.message))
else:
    print("nicos_oracle_reljobs.py: Connection is alive!")
...
print("nicos_oracle_reljobs: ",cmnd)
cursor.execute(cmnd,{'nname' : nname})
result = cursor.fetchall()
pprint(result)
#print "(name, type_code, display_size, internal_size, precision, scale, null_ok)"
#pprint(cursor.description)
if len(result) < 1: print("nicos_oracle_reljobs.py: absent nightly name: ",nname); sys.exit(1)   
#for row in result:
#    print row
row=result[-1]
nightly_id=row[0]
...
ts_j=""
relnmaster=""
print('nicos_oracle_reljobs.py: TS',ts," release name stamp: ",relnstamp)
...
print('nicos_oracle_reljobs.py: tstamps:',ts,ts_1)
cmnd="""
...
#        print ts
        if row[1] == ts:
            print("nicos_oracle_reljobs.py: relid found:",row[0],' XX ',row[1], ts)
...
#            print row
        ...
        if rowmax[1] > ts_2:
            print("nicos_oracle_reljobs.py: relid found:",rowmax[0],' XX ',rowmax[1],' > ', ts_2)
            relid_j=rowmax[0]; ts_j=rowmax[1]
#
if relid_j == "":
      print("nicos_oracle_reljobs.py: Error: relid for jobs table not found")
      sys.exit(1)
#print "TSJ",ts_j
...
if jid_res == '' :
    ...
    print("nicos_oracle_reljobs.py: MASTER jid candidate (base):", jid)
else:
    jid=jid_res
    jid_base=(int(jid)/1000)*1000
    jid_max=jid_base+999
    print("nicos_oracle_reljobs.py: jid from file:",jid,jid_base)
####
...
if lresult > 0:
  for row in result:
    print('nicos_oracle_reljobs.py: JID,RELID', row)
  rowmax=result[-1]
  jid = rowmax[0]
  relid_jj = rowmax[1]
...
if firstproj == 1 and jid_res == '':
    jid += 1
    relid_jj=relid_j 
print("nicos_oracle_reljobs.py: max jid:",jid," for relid:",relid_j,relid_jj," current ts:",ts_j," ROLE ",role_up) 
if relid_jj == "" or relid_j != relid_jj:
    print("nicos_oracle_reljobs.py: Warning: relid from jobs table differs from releases table",relid_j," xx ",relid_jj) 
#
if firstproj == 1:
  if jid_res == '':  
    cmnd="""
...
, :opt, :role, :kitb
, :cvmfs
, :t_val, :nicos_webarea, :nicos_http_build, :btool, :bvers, :buildarea, :copyarea, :target_branch, :gitmrhash)"""
    dict_p={'jid':jid,'nightly_id':nightly_id,'relid_j':relid_j,'arch':arch,'osys':osys,'comp':comp,'opt':opt,'role':role_up,'kitb':kitb,'cvmfs':cvmfs,'t_val':ts_j,'nicos_webarea':nicos_webarea,'nicos_http_build':nicos_http_build, 'btool' : btool, 'bvers' : bvers, 'buildarea' : buildarea, 'copyarea' : copyarea, 'target_branch' : target_branch, 'gitmrhash' : gitmrhash}
...
,cvmfs = :cvmfs
,webarea = :nicos_webarea
,webbuild = :nicos_http_build
,buildarea = :buildarea 
,copyarea = :copyarea
,gitbr = :target_branch where 
jid = :jid"""
    dict_p={'jid':jid_res,'kitb':kitb,'cvmfs':cvmfs,'nicos_webarea':nicos_webarea,'nicos_http_build':nicos_http_build,'buildarea':buildarea,'copyarea':copyarea,'target_branch':target_branch}
...
if lresult > 0:
    jid = rowmax[0]
    print("nicos_oracle_reljobs.py: job id for jobstat insertion identified:",jid)
else:
    print("nicos_oracle_reljobs.py: Error: absent job for relid:",relid_j)
    sys.exit(1)
hostnam=os.uname()[1]
prcid=os.getpid()
usid=os.getuid()
parid=os.getppid() 
#print "nicos_oracle_reljobs.py: PID,PPID,USERID", prcid, parid, usid
...
result = cursor.fetchall()
if len(result) == 0:
            print("nicos_oracle_reljobs.py: Error: absent project:",paname)
            sys.exit(1)
rowmax=result[-1]
...
result = cursor.fetchall()
if len(result) > 0:
           print("nicos_oracle_reljobs.py: Error: attempt to add existing jobstat row: jid,nid,relid,projid",jid,nightly_id,relid_j,projid_current)
           sys.exit(1)
#
procid='100000'
cmnd="""
...
dict_p={'jid':jid,'nightly_id':nightly_id,'relid_j':relid_j,'projid_c':projid_current,'lastpj':lastproj,'hostnam':hostnam,'procid':procid,'req':'NONE','stat':'START','t_val':ts_j,'nicos_suffix':nicos_suffix}
print("nicos_oracle_reljobs.py:", cmnd, dict_p)
cursor.prepare(cmnd)
cursor.setinputsizes(t_val=cx_Oracle.TIMESTAMP)
cursor.execute(None, dict_p)
